...
int displayFiles (fdDir * dirp, int flall, int fllong)
	...
	while (di != NULL) 
		{
		if ((di->name[0] != '.') || (flall)) //if not all and starts with '.' it is hidden
			{
			if (fllong)
				{
				fs_stat (di->name, &statbuf);
				printf ("%s    %9ld   %s\n", fs_isDir(di->name)?"D":"-", statbuf.st_size, di->name);
				}
			else
				{
				printf ("%s\n", di->name);
...
int cmd_cat (int argcnt, char *argvec[])
        ...
        do 
                {
                readcnt = b_read (testfs_src_fd, BufferPos, BUFFERLEN-1);
                BufferPos[readcnt] = '\0';
                printf("%s", BufferPos);
...
int cmd_cp (int argcnt, char *argvec[])
	...
	do 
		{
		readcnt = b_read (testfs_src_fd, BufferPos, BUFFERLEN);
		b_write (testfs_dest_fd, BufferPos, readcnt);
...
int cmd_cp2l (int argcnt, char *argvec[])
	...
	do 
		{
		readcnt = b_read (testfs_fd, BufferPos, BUFFERLEN);
		write (linux_fd, BufferPos, readcnt);
		} while (readcnt == BUFFERLEN);
	b_close (testfs_fd);
	close (linux_fd);
#endif
...
int cmd_cp2fs (int argcnt, char *argvec[])
	...
	do 
		{
		readcnt = read (linux_fd, BufferPos, BUFFERLEN);
		b_write (testfs_fd, BufferPos, readcnt);
		} while (readcnt == BUFFERLEN);
	b_close (testfs_fd);
	close (linux_fd);
#endif
