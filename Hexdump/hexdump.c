...
int processFile (char * filename, uint64_t startBlock, uint64_t numBlocks)
	...
	unsigned char * BufferPos = malloc (BUFSIZE);		//Allocate the read buffer
	...
	do
		...
		readbytes = read (fd, BufferPos, BUFSIZE);		//Read one block
		...
		for (int i = 0; i < loops; i++)				//Loop for each "Block" within one buffer read
			...
			for (int j = 0; j < BLOCKSIZE/16; j++)	//loop j lines for each block
				...
				if (offset + 16 > readbytes)
					...
					for (k = 0; k < readbytes - offset; k++)
						{
						printf ("%c", BufferPos[offset + k] < 32?'.':BufferPos[offset+k]);
						}
					printf("\n");
					...
					printf ("%06X: %02X %02X %02X %02X %02X %02X %02X %02X  %02X %02X %02X %02X %02X %02X %02X %02X | %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
						offset+position,
						BufferPos[offset + 0],BufferPos[offset + 1],BufferPos[offset + 2],BufferPos[offset + 3],
						BufferPos[offset + 4],BufferPos[offset + 5],BufferPos[offset + 6],BufferPos[offset + 7],
						BufferPos[offset + 8],BufferPos[offset + 9],BufferPos[offset + 10],BufferPos[offset + 11],
						BufferPos[offset + 12],BufferPos[offset + 13],BufferPos[offset + 14],BufferPos[offset + 15],
						BufferPos[offset + 0] < 32?'.':BufferPos[offset + 0],
						BufferPos[offset + 1] < 32?'.':BufferPos[offset + 1],
						BufferPos[offset + 2] < 32?'.':BufferPos[offset + 2],
						BufferPos[offset + 3] < 32?'.':BufferPos[offset + 3],
						BufferPos[offset + 4] < 32?'.':BufferPos[offset + 4],
						BufferPos[offset + 5] < 32?'.':BufferPos[offset + 5],
						BufferPos[offset + 6] < 32?'.':BufferPos[offset + 6],
						BufferPos[offset + 7] < 32?'.':BufferPos[offset + 7],
						BufferPos[offset + 8] < 32?'.':BufferPos[offset + 8],
						BufferPos[offset + 9] < 32?'.':BufferPos[offset + 9],
						BufferPos[offset + 10] < 32?'.':BufferPos[offset + 10],
						BufferPos[offset + 11] < 32?'.':BufferPos[offset + 11],
						BufferPos[offset + 12] < 32?'.':BufferPos[offset + 12],
						BufferPos[offset + 13] < 32?'.':BufferPos[offset + 13],
						BufferPos[offset + 14] < 32?'.':BufferPos[offset + 14],
						BufferPos[offset + 15] < 32?'.':BufferPos[offset + 15]);
					}
